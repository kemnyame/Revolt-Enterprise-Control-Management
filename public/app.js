const state={token:localStorage.getItem("revolt_token")||"",user:null,permissions:[],controls:[],assessments:[],evidence:[],findings:[],audit:[],users:[],dashboard:null,page:"overview"};
...
async function loadAll(){
  ...
  try{
    const jobs=[api("/api/dashboard"),api("/api/controls"),api("/api/assessments"),api("/api/evidence"),api("/api/findings"),has("audit.read")?api("/api/audit"):Promise.resolve([]),has("users.read")?api("/api/users"):Promise.resolve([])];
    const [dashboard,controls,assessments,evidence,findings,audit,users]=await Promise.all(jobs);
    Object.assign(state,{dashboard,controls,assessments,evidence,findings,audit,users});renderAll();
  }catch(err){toast(err.message)}finally{document.body.classList.remove("loading")}
}
function renderAll(){renderDashboard();renderControls();renderAssessments();renderEvidence();renderFindings();renderAudit();renderUsers();populateFilters()}
function go(page){
  ...
  const titles={overview:"Control Overview",controls:"Control Library",assessments:"Control Assessments",evidence:"Evidence Register",findings:"Findings & Remediation",audit:"Audit Trail",users:"Users & Roles"};
...
async function refresh(parts){
  const map={dashboard:["dashboard","/api/dashboard"],controls:["controls","/api/controls"],assessments:["assessments","/api/assessments"],evidence:["evidence","/api/evidence"],findings:["findings","/api/findings"],audit:["audit","/api/audit"],users:["users","/api/users"]};
...
async function submitForm(e){
  ...
  if(["assessment","evidence","finding"].includes(kind)&&data.control_id)data.control_id=Number(data.control_id);
  ...
  const endpoints={control:["/api/controls",["controls","dashboard","audit"]],assessment:["/api/assessments",["assessments","controls","dashboard","audit"]],evidence:["/api/evidence",["evidence","controls","dashboard","audit"]],finding:["/api/findings",["findings","controls","dashboard","audit"]],user:["/api/users",["users","audit"]]};
