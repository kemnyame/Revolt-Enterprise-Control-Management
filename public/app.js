...
function renderDashboard(){
  ...
  $("#overviewFindings").innerHTML=open.length?'<div class="table-card"><table><thead><tr><th>Issue</th><th>Control</th><th>Severity</th><th>Owner</th><th>Due</th><th>Status</th></tr></thead><tbody>'+open.map(x=>'<tr class="click-row" data-overview-finding="'+x.id+'"><td class="control-name"><b>'+esc(x.title)+'</b></td><td>'+esc(x.control_code||"—")+'</td><td>'+tag(x.severity)+'</td><td>'+esc(x.owner||"Unassigned")+'</td><td>'+fmtDate(x.due_date)+'</td><td>'+tag(x.status)+'</td></tr>').join("")+'</tbody></table></div>':'<div class="empty-state">No open issues.</div>';
  $$("[data-overview-finding]").forEach(row=>row.onclick=()=>{go("findings");openFinding(Number(row.dataset.overviewFinding))});
}
...
async function openControl(id){
  try{
    ...
    '<div class="detail-grid"><div class="detail-card"><small>CONTROL AREA</small><b>'+esc(c.category)+'</b></div><div class="detail-card"><small>OWNER / FUNCTION</small><b>'+esc(c.owner||"Unassigned")+'</b></div><div class="detail-card"><small>ASSIGNED OFFICER</small><b>'+esc(c.assigned_user_name||"Unassigned")+'</b></div><div class="detail-card"><small>FREQUENCY</small><b>'+esc(c.frequency)+'</b></div><div class="detail-card"><small>RISK</small><b>'+tag(c.risk_level)+'</b></div><div class="detail-card"><small>FRAMEWORK</small><b>'+esc(c.framework_ref||"—")+'</b></div><div class="detail-card"><small>EVIDENCE EXPECTATION</small><b>'+esc(c.evidence_required||"—")+'</b></div></div>'+
